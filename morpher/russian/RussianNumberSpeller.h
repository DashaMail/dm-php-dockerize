...
class StringBuilder : public std::tostringstream
{
	bool spaceRequired;
...
public:
    ...
    void Append (std::tstring s) const
    {
        _sb << s;
    }

    void Append (std::tstring s, size_t start, size_t count) const
    {
    ...
    std::tstring SpellOne () const
    {
        ...
        return std::tstring (_T("од")) + forms [(int) _gCaseWithAnimacy];
    }
    ...
    void Spell10to19 (int lastDigit) const
    {
        ...
        else
        {
            ...
            {
                std::tstring s = zeroToNine [lastDigit];
...
struct SpellResult
{
	...
	SpellResult (std::tstring number, std::tstring unit) : errorCode (SpellErrorCode_Success), number (number), unit (unit) {}
...
static ParsePositiveNumberResult ParsePositiveNumber (const std::tstring & number)
{
	...
	std::tstring whole = number.substr (0, i);
	if (whole.empty ()) whole = _T("0");

	std::tstring fraction = (unsigned) i == number.length () ? _T("") : number.substr (i+1);
...
public:
	...
	SpellResult Spell (const std::tstring & number, std::tstring unit, int padeg) const
	{
		if (number.empty ()) return SpellResult (SpellErrorCode_NumberIsEmpty);

		if (padeg < 0 || padeg >= 7) throw std::runtime_error("Invalid case.");
		...
		std::tstring whole = parseResult.integer;
		std::tstring fraction = parseResult.fraction;

		if (whole.empty ()) return SpellResult (SpellErrorCode_NumberIsNotANumber);

		if (! fraction.empty ())
		{
            std::tstring unit_rod = GenitiveSingular (unit);

            if (unit_rod.empty ()) return SpellResult (SpellErrorCode_DeclensionError);

            std::tstring wholeUnit = _T("целая"); // TODO: для украинского

			SpellResult wholeResult = Spell (whole, wholeUnit, padeg);

			if (wholeResult.errorCode != 0) return wholeResult;

            std::tstring fractionalUnit = Ordinal (fraction.length ()); // TODO: для украинского
        ...
        if (whole == _T("0"))
        {
            std::tstring unit_rod_mnozh = GenitivePlural (unit);
		...
		for (;;)
		{
			...
			if (triple > 0)
			{
                std::tstring rankUnit = GetUnit (rank - 1);
	...
	void SpellTriple (StringBuilder & sb, int n, std::tstring * unit, int padeg) const
	{
        auto lang = std::unique_ptr <ICaseSpeller> (CreateSpeller (sb, n, padeg, unit));
    ...
    virtual ICaseSpeller * CreateSpeller (StringBuilder & sb, int n, int padeg, std::tstring * unit) const = 0;

    /// <summary> Получить название класса с заданным рангом. </summary>
    /// <param name="rank">0 = тысяча, 1 = миллион, 2 = миллиард и т.д. </param>
    virtual std::tstring GetUnit (size_t rank) const = 0;
	virtual const TCHAR * SpellZero (int padeg) const = 0;
	virtual std::tstring GenitivePlural (const std::tstring & unit) const = 0;

	std::tstring Ordinal (size_t exponent) const
	{
		std::tstring stem = OrdinalStem (exponent);
	...
	std::tstring OrdinalStem (size_t exponent) const
    {
        switch (exponent)
        {
        ...
        std::tstring unit = rank == 0 ? _T("тысяч") : GetUnit (rank);
	...
	virtual std::tstring GenitiveSingular (const std::tstring & unit) const = 0;
};
...
public:
	...
	RussianNumberSpeller (const IMorpher * morpher)
		: morpher (morpher)
	{
		if (!morpher) throw std::runtime_error ("morpher is nullptr.");
	}

	virtual const TCHAR * Minus () const override { return _T("минус"); }

	virtual std::tstring GenitiveSingular (const std::tstring & unit) const override
	{
	...
	virtual std::tstring GenitivePlural (const std::tstring & unit) const override
	{
		PhraseWrapper phraseWrapper = ParseUnit(unit);
	...
	virtual std::tstring GetUnit (size_t rank) const override
    {
    ...
    static std::tstring GetUnit (GramCase gCase, int n, const Phrase * parse)
    {
        bool plural;
	...
	static std::list <GC::Ptr <const IPhrasePart> > MakeList (const GC::Ptr <const IPhrasePart> & elem)
	{
		std::list <GC::Ptr <const IPhrasePart> > list;
	...
	Phrase* ParseUnit(const std::tstring & unit) const
	{
	...
	virtual ICaseSpeller * CreateSpeller (StringBuilder & sb, int n, int padeg, std::tstring * unit) const override
	{
		PhraseWrapper phraseWrapper = ParseUnit(*unit);
