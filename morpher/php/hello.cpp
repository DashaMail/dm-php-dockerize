...
#define _ADD_NEXT_INDEX_STRING(arr, return_value) add_next_index_string(arr, return_value, 1); 
#else
#define _RETURN_STRING(str) RETURN_STRING(str)
#define _ADD_NEXT_INDEX_STRING(arr, return_value) add_next_index_string(arr, return_value); 
#endif
...
typedef std::tstring (Paradigm::*ParseMemFun) () const;
...
static string ToUTF8 (const wstring & result)
{
	...
	std::unique_ptr<char> utf8_result (new char [utf8_result_buffer_size]);
...
{
	std::unique_ptr<wchar_t> wcsName (new wchar_t [utf8_name_size_in_bytes + 1]);
...
public:
		...
		size_t utf8_name_size_in_bytes)
	{
		...
		std::unique_ptr<Parse> phrase = morpher.parse (wcsName);
...
PHP_FUNCTION(morpher_split_fio)
{
	const char *utf8_fio;
	size_t utf8_name_size_in_bytes = 0;
	...
	std::wstring fioString = FromUTF8(utf8_fio, utf8_name_size_in_bytes);

	std::unique_ptr<Parse> phrase = morpher.parse(fioString, Attribute_Fio);
	
	std::wstring name;
	std::wstring surname;
	std::wstring patronymic;
	
	if (phrase && phrase->fio())
	{
		name = phrase->fio()->GetName();
		surname = phrase->fio()->GetSurname();
		patronymic = phrase->fio()->GetPatronymic();
	}
	
	std::string utf8_name = ToUTF8(name);
	std::string utf8_surname = ToUTF8(surname);
	std::string utf8_patronymic = ToUTF8(patronymic);

	array_init(return_value);
	_ADD_NEXT_INDEX_STRING(return_value, (char*)utf8_surname.c_str());
	_ADD_NEXT_INDEX_STRING(return_value, (char*)utf8_name.c_str());
	_ADD_NEXT_INDEX_STRING(return_value, (char*)utf8_patronymic.c_str());
}