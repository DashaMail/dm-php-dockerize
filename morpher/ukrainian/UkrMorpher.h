...
namespace UkrMorpher
{
	...
	private:
		...
		static tstring GetEndings (const tstring & word, bool isMasculine, int noun, bool * isAdjective)
		{
			*isAdjective = false;
			...
			if (HasEnding (word, _T("а"))) 
				return _T("а и і у ою і о"); // Ганна
			if (HasEnding (word, _T("я"))) 
				return isMasculine || HasEnding(word, _T("ія"))
					? _T("я і і ю ею і є") // Ілля, Вікторія
					: HasEnding(word, _T("ря"))
						? _T("я і і ю ею і е")  // Катря, Зоря
						: _T("я і і ю ею і ю"); // Наталя, Ася
